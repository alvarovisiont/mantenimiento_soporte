...
<div class="modal fade modal-slide-in-right" aria-hidden="true" role="dialog" tabindex="-1" id="modal-eliminar-{{$row->id}}">
{{Form::Open(array('action'=>array('SoportesController@destroy',$row->id), 'method' =>'delete','id' =>'form'))}}
{{--<input type="hidden" name="_token" value="{{ csrf_token }}" id="token">--}}
<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			...
			<h4 class="modal-title">Elminar Usuario {{ $row->nombre_completo }} </h4>
		</div>
		<div class="modal-body">
			<h6>¿SEGURO DESEA ELIMINAR ESTE USUARIO?</h6>
		</div>
		<div class="modal-footer">
			<button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>
